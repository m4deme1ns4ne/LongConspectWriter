...
**Структурированный вывод (JSON Schema).** Три агента — `AgentGlobalPlanner`, `AgentGraphPlanner`, `AgentExtractor` — используют `response_format` в llama.cpp, что гарантирует синтаксически корректный JSON-вывод без постобработки.
...
|:---------:|----------|-----------------|:----:|
| **P1** | Модульная семантическая архитектура | Разбивка на тематические блоки, иерархия разделов, навигация | 0–10 |
| **P2** | Обезличивание | Отсутствие мета-нарратива — отсылок к лектору, аудитории, процессу лекции | 0–10 |
| **P3** | Математический формализм | Корректность LaTeX-нотации, оформление кода в `monospace` | 0–10 |
| **P4** | Визуализация | Наличие схем, ASCII-диаграмм, таблиц там, где визуализация уместна | 0–10 |
| **P5** | Фактическая точность | Критические ошибки (неверные формулы), значимые ошибки, незначительные | 0–10 |
| **P6** | Полнота охвата | Ключевые темы транскрипта, представленные с достаточной глубиной | 0–10 |
| **P7** | Самодостаточность | Читаемость без прослушивания лекции, контекст и объяснения «зачем» | 0–10 |
