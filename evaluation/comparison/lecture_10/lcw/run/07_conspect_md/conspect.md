...
Значение $ -13{,}6~\text{эВ} $ соответствует энергии ионизации атома водорода (при $ n=1 $, $ Z=1 $) и называется **постоянной Риттберга**.
...
а кинетическая — положительная, $ E_{\text{кин}} = \frac{mv_n^2}{2} $. Подстановка выражений для скорости $ v_n = \frac{Ze^2}{n\hbar} $ и постоянная величина в итоге даёт значение $ -13{,}6~\text{эВ} $ — это энергия ионизации атома водорода при $ n=1 $, называемая **постоянной Риттберга**.  
...
Разность энергий между уровнями $ E_n - E_m = \frac{hc}{\lambda_{nm}} $, где $ h $ — постоянная Планка, $ c $ — скорость света. Если $ n > m $, происходит излучение фотона; если $ n < m $ — поглощение. Частоты переходов измеряются спектроскопически: серия Бальмера ($ n=2 $) соответствует видимому диапазону длин волн, серия Лаймана ($ n=1 $) — ультрафиолету. Экспериментальные данные полностью согласуются с предсказаниями модели Бора.  
