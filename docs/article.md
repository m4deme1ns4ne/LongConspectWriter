...
Меня зовут Волжанин Александр, я закончил университет в этом году и эта статья по моей ВКР. В этой статье я бы хотел предложить метод обработки большого кол-ва токенов для генерации конспектов с помощью малых языковых моделей на потребительском GPU. Я бы не относился к этой статье слишком серьёзно: исследованием её можно назвать лишь с очень большой натяжкой. Скорее это инженерный проект с выводом и гипотезами. Буду рад любой обратной связи.

Оговорюсь: к части смежных работ (RAPTOR, Chain of Agents и др.) я пришёл уже после того, как собрал пайплайн, - поэтому ряд решений в нём переоткрыт независимо, а не заимствован. На научную новизну это, разумеется, не влияет; скорее это говорит о том, что подобные приёмы естественно возникают под выбранными ограничениями.

## Аннотация

Автоматическая генерация структурированных академических конспектов из аудиозаписей лекций по точным и естественным наукам затруднена для локальных малых языковых моделей (small language models, SLM). Транскрипт лекции продолжительностью ≈1,5 ч составляет около 15–20 тыс. токенов и формально умещается в контекстное окно современных локальных SLM, однако при обработке такого контекста single-call SLM систематически деградируют: теряют фрагменты из середины последовательности, не удерживают структуру и галлюцинируют термины и формулы. Это проявление эффекта [Lost in the Middle](https://doi.org/10.1162/tacl_a_00638): точность извлечения информации описывает U-образную кривую — высока на краях контекста и падает в середине; в наших условиях используются SLM, поведение которых так же описано в статье, и оно характеризуется выраженным забыванием не только из середины, но также и из начала контекста. Более того, на бюджете 8 ГБ VRAM single-call длинного транскрипта практически неприменим*, что делает декомпозицию не оптимизацией, а необходимым условием работоспособности. Далее в статье я не буду больше упоминать этот факт.

(*Формально single-call можно уместить в 8 ГБ, понизив число параметров модели или применив более агрессивную квантизацию. В пределах этого бюджета мы фиксируем модель максимальной параметрической ёмкости при выбранной разрядности квантизации; компромисс «меньшая ёмкость модели в обмен на single-call по более длинному контексту» систематически не исследовался.)
...
8. [Пример работы](#example)
9. [Ограничения](#limitations)
...
Транскрипт лекции продолжительностью ≈1,5 ч имеет объём порядка 15–20 тыс. токенов. Все текстовые агенты пайплайна (планировщики, синтезатор, экстрактор, планировщик визуализаций) используют единую модель [T-lite-it-2.1](https://huggingface.co/t-tech/T-lite-it-2.1-GGUF) в квантизации GGUF Q5_K_M (файл `T-lite-it-2.1-Q5_K_M.gguf`) с контекстным окном 8192 токена (у планировщика визуализаций — 4096) через [llama-cpp-python](https://github.com/abetlen/llama-cpp-python) (версия 0.3.20); агент генерации кода визуализаций — [Qwen2.5-Coder-7B-Instruct](https://huggingface.co/Qwen/Qwen2.5-Coder-7B-Instruct-GGUF) в квантизации Q6_K (контекст 4096 токенов). Генерация у текстовых агентов жадная (температура 0, `min_p` 0,15, `repeat_penalty` 1,05; у планировщика визуализаций — 1,1). Общая схема пайплайна приведена на рис. 1.
...
<p align="center">
<img src="../assets/mermaid-diagram-2026-06-05T23-14-56.svg" width="50%" alt="Общая архитектура мультиагентного пайплайна LCW">
</p>
...
<p align="center">
<img src="../assets/Снимок экрана 2026-06-16 221643.png" width="100%" alt="Баллы по семи парадигмам P1–P7 по каждой лекции: LCW и Gemini 3.1 Pro">
</p>
...
Чтобы дополнить агрегированные оценки качественной иллюстрацией, ниже разобрана одна из наиболее удачных для LCW лекций — «Основные понятия химии» (лекция 9, домен «химия»). По семи парадигмам она набрала 8,00/10 против 9,71/10 у эталона (≈82%) — это один из лучших результатов LCW в выборке, поэтому пример показывает не средний, а близкий к потолку метода случай. Профиль оценок по парадигмам приведён на рис. 6.

<p align="center">
<img src="../assets/example_lecture09_profile.png" width="55%" alt="Профиль оценок LCW и Gemini 3.1 Pro по семи парадигмам для лекции 9">
</p>

*Рис. 6. Профиль оценок по семи парадигмам для лекции «Основные понятия химии» (LCW против Gemini 3.1 Pro). LCW почти смыкается с эталоном по полноте (P6), самодостаточности (P7) и структуре (P1) и заметно отстаёт по визуализации (P4) и строгости (P3).*

Содержательно конспекты близки: оба верно воспроизводят классификацию способов познания и физические границы химии. Различие проявляется прежде всего в форме представления. LCW излагает материал связной прозой:
...
Эталон ту же классификацию оформляет таблицей:
...
а физические масштабы химии Gemini дополнительно изображает текстовой ASCII-схемой по трём осям (энергия, время, расстояние), тогда как LCW описывает те же масштабы числовым перечислением в тексте. Этот контраст наглядно отражает разрыв, зафиксированный в разделе 7 количественно: при сопоставимой фактической полноте локальная модель уступает эталону в структурно-визуальном оформлении (P1, P4) — в выборе табличного или схематического представления там, где оно уместнее прозы.
