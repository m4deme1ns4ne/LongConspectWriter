---
title: "LongConspectWriter: автоматическая генерация структурированных конспектов лекций на потребительском GPU"
author: "Александр Волжанин"
date: "июнь 2026"
abstractlabel: "Аннотация"
links: "[GitHub](https://github.com/m4deme1ns4ne/LongConspectWriter) · [Статья на Habr](https://habr.com/ru/articles/1049640/)"
abstract: |
  ...
  *Выборка на тестах составила всего 10 лекций, поэтому делать статистические выводы нельзя: число 78% следует трактовать как предварительный результат для построения гипотезы. Исходный код, датасет тестирования и тесты доступны под лицензией MIT на [GitHub](https://github.com/m4deme1ns4ne/LongConspectWriter).*
---
...
Транскрипт лекции продолжительностью ≈1,5 ч имеет объём порядка 15–20 тыс. токенов. Все текстовые агенты пайплайна (планировщики, синтезатор, экстрактор, планировщик визуализаций) используют единую модель [T-lite-it-2.1](https://huggingface.co/t-tech/T-lite-it-2.1-GGUF) в квантизации GGUF Q5\_K\_M (файл `T-lite-it-2.1-Q5_K_M.gguf`) с контекстным окном 8192 токена (у планировщика визуализаций — 4096) через [llama-cpp-python](https://github.com/abetlen/llama-cpp-python) (версия 0.3.20); агент генерации кода визуализаций — [Qwen2.5-Coder-7B-Instruct](https://huggingface.co/Qwen/Qwen2.5-Coder-7B-Instruct-GGUF) в квантизации Q6\_K (контекст 4096 токенов). Генерация у текстовых агентов жадная (температура 0, `min_p` 0,15, `repeat_penalty` 1,05; у планировщика визуализаций — 1,1). Общая схема пайплайна приведена на рис. 1.
...
![Общая архитектура мультиагентного пайплайна LCW.](fig1_architecture.pdf){width=58%}
...
| 6  | Функция и её график. Множества целых и рациональных чисел | Мат. анализ | 6,71 | 8,71 | 77% |
...
: Баллы по 10 лекциям с подытогами по доменам и общим средним (LCW / Gemini 3.1 Pro / % от эталона).

При двух лекциях на домен порядок доменов иллюстративен и не интерпретируется.

Детализация по семи парадигмам приведена в табл. 2 (формат ячеек — LCW / Gemini 3.1 Pro; номера лекций соответствуют табл. 1).

| Лекция | P1 | P2 | P3 | P4 | P5 | P6 | P7 | Среднее |
|:------:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:-------:|
...
: Баллы по семи парадигмам P1–P7 для каждой из 10 лекций и среднее (LCW / Gemini 3.1 Pro).
